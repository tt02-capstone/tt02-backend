...
public class CartController {
    ...
    @PostMapping("/addItemToCart/{userId}/{itemId}")
    public ResponseEntity<Long> addItemToCart(@PathVariable Long userId, @PathVariable Long itemId, @RequestBody CartBooking cartBooking) throws NotFoundException {
        Long id = cartService.addItemToCart(userId, itemId, cartBooking);
        return ResponseEntity.ok(id);
    }
    ...
    public ResponseEntity<List<Long>> checkout(@PathVariable String user_type,
        ...
        List<Long> createdBookingIds =  cartService.checkout(user_type,tourist_email, payment_method_id, totalPrice, checkoutRequest.getBooking_ids(), checkoutRequest.getPriceList(), checkoutRequest.getSelectedDeliveryType(), checkoutRequest.getDeliveryAddress());
